...
function CarouselSimple({ images, setPage, defaultImageWidth }: SimpleCarouselProps) {
                ...
                {images.map((image, index) => {
                    ...
                    return (
                        <div key={`${image.id}-simple`} className="flex flex-row mr-4 lg:flex-none lg:basis-[calc(25%_-_16px)] md:flex-none md:basis-[calc(50%_-_15px)] md:w-1/2 sm:w-full h-50 shrink-0">
                            <div ref={fetcher ? observeRef : null} data-testid={fetcher ? 'fetcher' : null} className="relative flex flex-col justify-end w-full h-full p-4 shrink-0 rounded-2xl bg-cover bg-center group" style={{backgroundImage: `url(https://picsum.photos/id/${image.id}/${defaultImageWidth}/${scaledHeight})`}}>
                            ...
                            {index === images.length - 1 && <div aria-label="placeholder" className="flex flex-col justify-end w-full h-50 ml-4 bg-gray-300 opacity-0 shrink-0 rounded-2xl animate-[PlaceholderPulse_2s_ease_infinite]"></div>}
                        </div>  
                        );
                })}
            </div>
